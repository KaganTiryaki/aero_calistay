...
import stil from "./revak.module.css";

// three ~150KB gzip: ilk bundle'a girerse projenin <150KB bütçesini tek başına
// yer → ayrı chunk + yalnız istemci.
const YuruyenIsikSahnesi = dynamic(
  () => import("./YuruyenIsikSahnesi").then((m) => m.YuruyenIsikSahnesi),
  { ssr: false },
);
...
export function Revak({
  ...
  // Sahne her karede ışığın hangi göze uğradığını buraya yolluyor. setState YOK:
  // doğrudan custom property yazılıyor, React render'ı hiç dönmüyor.
  ...
  return (
    <main className={stil.kok}>
      <YuruyenIsikSahnesi sinif={stil.tuval} disiplinler={disiplinler} bildir={bildir} />
