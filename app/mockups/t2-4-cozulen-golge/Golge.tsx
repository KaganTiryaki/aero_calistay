...
export function Golge({
  ...
  // Sahne her karede çözülme değerini buraya yolluyor. setState YOK: doğrudan
  // custom property yazılıyor, React render'ı hiç dönmüyor.
  const bildir = useCallback((deger: number[]) => {
    for (let i = 0; i < deger.length; i++) {
      seritRef.current[i]?.style.setProperty("--cozulme", deger[i].toFixed(3));
    }
  }, []);
  ...
  return (
    <main className={stil.kok}>
      <GolgeSahnesi
        sinif={stil.tuval}
        disiplinAdet={disiplinler.length}
        bildir={bildir}
      />
